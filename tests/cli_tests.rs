...
fn test_cli_add_and_get() {
    ...
    if fs::remove_file(json_path).is_err() {
        eprintln!("Impossible de supprimer {}", json_path);
    }
}

#[test]
fn test_cli_convert_to_french() {
    let json_path = "addresses.json"; // Chemin du fichier JSON

    // Assurer que le fichier est supprimé avant le test
    let _ = fs::remove_file(json_path); // Ignore les erreurs si le fichier n'existe pas

    // Ajoute une adresse au format ISO20022
    let mut cmd = Command::cargo_bin("fraddriso20022").unwrap();
    let add_output = cmd
        .arg("add")
        .arg("--kind")
    ...
    // Convertir l'adresse ISO20022 en FrenchAddress
    ...
    cmd_convert
        ...
        .success()
        .stdout(contains("DURAND SA"))
        .stdout(contains("Purchasing Department"))
        .stdout(contains("Industrial Zone"))
        .stdout(contains("22BIS Rue des Fleurs"))
        .stdout(contains("BP 40122"))
        .stdout(contains("33506 LIBOURNE CEDEX"))
